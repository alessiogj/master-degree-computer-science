...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        % Creazione dei nodi
        \node[oval node=blue!20] (A) at (0,3) {Obiettivi};
        \node[oval node=green!20] (B) at (-3,-1) {Agenti};
        \node[oval node=red!20] (C) at (3,-1) {Scenario};

        % Creazione degli archi con etichette
        \draw (A) -- (B) node[midway, left] {Responsabilità};
        \draw (A) -- (C) node[midway, right] {Copertura};
