...
\begin{tcolorbox}[title = Distinguisher]
  Un \textbf{distinguisher} è un algoritmo binario $\mathcal{D} \in \texttt{PPT}$ che restituisce $0$ o $1$. Il distinguisher prende in input un evento che soddisfa o meno una certa proprietà e 
...
di $m$ quando il security parameter vale $k$. Un sistema di codifica nascone $m_1$ a $m_2$ a $\mathcal{D}$ quando:
\begin{equation}
...
\end{equation}
$E$ nasconde informazioni a $\mathcal{D}$ se per ogni $m_1$ a $m_2$ $E$ nasconde $m_1$ a $m_2$ a $\mathcal{D}$. 
$E$ nasconde se per ogni $\mathcal{D} \in \texttt{PPT}$ $E$ nasconde $m_1$ a $m_2$ a $\mathcal{D}$.
\begin{proof}
...
l'algoritmo $\mathcal{D}$ sul risultato.
\end{proof}