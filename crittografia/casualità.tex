...
Per arrivare a questo punto impareremo a costruire \textbf{numeri pseudo-casuali
...
moneta.
\[
  \bigg| \probP[Coin_a = 0] - \frac{1}{2} \bigg| < k^{-\omega(1)}
\]
...
\[
  \texttt{LSB}(\sqrt{x}) \textit{ è un hardcore predicate}
\]
...
\begin{enumerate}
    ...
    \item Sia $\mathcal{D}\in \texttt{PPT}$ un algoritmo che, su input $1^k$ (\textit{il 
